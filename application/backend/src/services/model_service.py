...
class ModelService:
    ...
    def get_backend_io_spec(backend_dir: Path) -> BackendIOSpec | None:
        ...
        input_names = ModelService._parse_string_list(manifest.get("input_names"))
        output_names = ModelService._parse_string_list(manifest.get("output_names"))

        if not input_features and not output_features and not input_names and not output_names:
            return None
