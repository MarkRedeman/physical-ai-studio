...
- `api/imports.py` (outside this folder): HTTP endpoints
...
- `source_hint`: usually `auto` (recommended), or `lerobot_v3`

Response:

- Returns `Job` with `payload.step = awaiting_upload`
- Save `job_id`
...
- Job is updated to `status=pending`, `payload.step=uploaded`
...
- `status=pending`
- `payload.step=waiting_for_user_input`
- `payload.validation_report`
- `payload.dataset_manifest_draft`

The `dataset_manifest_draft.schema` field is populated with a **recording schema** inferred
...
Cancellation is allowed before full import starts. Once import reaches execution steps (e.g. importing/registering), cancellation is rejected.
...
- If `source_hint != auto`, worker tries matching adapter first
- If `source_hint == auto`, worker scans adapters and uses first `detect(...) == True`
...
1. **Upload endpoint** (`api/imports.py`) – checks the **cache dir** (`$STORAGE_DIR/cache/imports/datasets/`) before writing the incoming archive.
