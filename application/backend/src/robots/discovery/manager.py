...
from schemas.robot import RobotType, SO101BimanualRobot
...
class DiscoveryManager:
    ...
    async def is_robot_online(self, robot: Robot) -> bool:
        ...
        if robot.type in {RobotType.SO101_BIMANUAL_FOLLOWER, RobotType.SO101_BIMANUAL_LEADER}:
            if not isinstance(robot, SO101BimanualRobot):
                return False
            available_serials = {cs.serial_number for cs in self.serial_manager.robots}
            return (
                robot.payload.serial_number_left in available_serials
                and robot.payload.serial_number_right in available_serials
            )
