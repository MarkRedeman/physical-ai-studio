...
class DatasetImportWorker(BaseProcessWorker):
    ...
    async def _process_job(self, job: DatasetImportJob) -> None:
        ...
        try:
            ...
            logging_ctx = job_logging_ctx(job_id=str(job_id))
        except Exception:
            logging_ctx = nullcontext()
        ...
        with logging_ctx:
            ...
            payload = job.payload
            archive_ref = payload.archive_staging_id or "<none>"
            logger.info(
            ...
            try:
                if payload.step == ImportStep.UPLOADED:
                    ...
                    pre_commit_handled = await self._run_commit(job_id, job.project_id, payload)
    ...
    async def _run_detection(self, job_id: UUID, _project_id: UUID, payload: DatasetImportJobPayload) -> None:
        ...
        if payload.dataset_manifest_draft is None:
            payload.dataset_manifest_draft = selected_adapter.parse_to_draft_manifest(archive=archive, payload=payload)
        ...
        report = selected_adapter.validate_pre_finalize(manifest=manifest, payload=payload)
        logger.info(
            "Pre-finalize validation report for job_id='{}': is_valid={}, required_user_inputs={}, blocking_errors={}",
            job_id,
            report.is_valid,
    ...
    async def _run_commit(self, job_id: UUID, project_id: UUID, payload: DatasetImportJobPayload) -> bool:
        ...
        if not pre_commit_report.is_valid:
            ...
            error_messages = "; ".join(e.message for e in pre_commit_report.blocking_errors)
