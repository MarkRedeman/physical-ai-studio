...
def _is_dataset_import_upload(request: Request) -> bool:
    path = request.url.path
    return request.method.upper() == "PUT" and "/imports/datasets/" in path and path.endswith(":upload")
...
async def upload_size_guard_middleware(request: Request, call_next: RequestResponseEndpoint) -> Response:
    ...
    if _is_dataset_import_upload(request):
