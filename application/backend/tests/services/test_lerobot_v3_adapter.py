...
def test_lerobot_v3_parse_manifest_reads_stats_from_nested_root_zip(tmp_path: Path) -> None:
    ...
    safe_archive = SafeZipArchive(archive_path, max_uncompressed_bytes=5 * 1024 * 1024 * 1024)
    manifest = adapter.parse_to_draft_manifest(safe_archive, payload=MagicMock())
...
def test_lerobot_v3_parse_manifest_prefers_uploaded_archive_name_for_suggested_name(tmp_path: Path) -> None:
    adapter = LeRobotV3Adapter()
    ...
    payload = MagicMock()
    payload.uploaded_archive_name = "warehouse-run-01.zip"

    safe_archive = SafeZipArchive(archive_path, max_uncompressed_bytes=5 * 1024 * 1024 * 1024)
    manifest = adapter.parse_to_draft_manifest(safe_archive, payload=payload)

    assert manifest.suggested_name == "warehouse-run-01"
