...
def test_write_model_card_includes_root_manifest_without_export_dirs(tmp_path) -> None:
    _write_manifest(
        tmp_path / "manifest.json",
        policy="act",
        artifacts={"torch": "exports/torch/act.pt"},
        input_features=[_feature("state", "STATE", [6], "float32")],
    )
    ...
    readme = readme_path.read_text(encoding="utf-8")
    assert "| torch | `exports/torch/act.pt` | Canonical checkpoint and Python inference |" in readme
    assert "| state | STATE | [6] | float32 |" in readme
    assert '"state": np.random.rand(1, 6).astype(np.float32)' in readme


def test_write_model_card_uses_policy_specific_copy(tmp_path) -> None:
    _write_manifest(tmp_path / "manifest.json", policy="act", artifacts={"torch": "exports/torch/act.pt"})

    readme_path = ModelManifestService.write_model_card(tmp_path)

    readme = readme_path.read_text(encoding="utf-8")
    assert "# Action Chunking Transformer (ACT)" in readme
    assert "imitation-learning policy" in readme
    assert "task instructions" in readme
    assert "language prompts" not in readme


def test_write_model_card_uses_language_conditioned_default_copy(tmp_path) -> None:
    _write_manifest(tmp_path / "manifest.json", policy="pi05", artifacts={"torch": "exports/torch/model.pt"})

    readme_path = ModelManifestService.write_model_card(tmp_path)

    readme = readme_path.read_text(encoding="utf-8")
    assert "# Pi0.5" in readme
    assert "Pi0.5 is a vision-language-action policy" in readme
    assert "task phrasing" in readme
    assert "language prompts" in readme


def test_write_model_card_handles_unknown_policy(tmp_path) -> None:
    _write_manifest(tmp_path / "manifest.json", policy="custom_policy", artifacts={"torch": "exports/torch/model.pt"})

    readme_path = ModelManifestService.write_model_card(tmp_path)

    readme = readme_path.read_text(encoding="utf-8")
    assert "# custom_policy" in readme
    assert "custom_policy is a vision-language-action policy" in readme
    assert "**Policy:** custom_policy" in readme


def test_write_model_card_generates_observation_samples_for_common_types(tmp_path) -> None:
    _write_manifest(
        tmp_path / "manifest.json",
        policy="act",
        artifacts={"torch": "exports/torch/act.pt"},
        input_features=[
            _feature("task", "TEXT", [1], "string"),
            _feature("episode_index", "INDEX", [], "int64"),
            _feature("temperature", "SCALAR", [], "float32"),
            _feature("state", "STATE", [6], "float32"),
        ],
    )
    ...
    readme = readme_path.read_text(encoding="utf-8")
    assert '"task": ["sample task description"]' in readme
    assert '"episode_index": np.zeros((), dtype=np.int64)' in readme
    assert '"temperature": np.array(0.0, dtype=np.float32)' in readme
    assert '"state": np.random.rand(1, 6).astype(np.float32)' in readme
...
def _write_manifest(
    ...
    *,
    ...
    input_features: list[dict] | None = None,
    output_features: list[dict] | None = None,
) -> None:
...
def _feature(name: str, ftype: str, shape: list[int], dtype: str) -> dict:
    return {
        "class_path": "physicalai.inference.data.features.InferenceFeature",
        "init_args": {
            "ftype": ftype,
