...
@pytest.mark.anyio
async def test_discover_job_sources_includes_dataset_import_name(tmp_path) -> None:
    jobs_dir = tmp_path / "jobs"
    jobs_dir.mkdir(parents=True)

    job_id = uuid4()
    file_path = jobs_dir / f"{job_id}.log"
    file_path.write_text("hello import")
    ...
    job = DatasetImportJob(
        ...
        payload={
            ...
            "uploaded_archive_path": "/tmp/my-dataset.zip",
            "source_hint": "auto",
    ...
    assert len(job_sources) == 1
    assert job_sources[0].id == f"job-{job_id}"
    assert job_sources[0].name == "Import: my-dataset.zip"
