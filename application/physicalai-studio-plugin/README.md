...
class CatalogRobotFactory(Protocol):
    ...
    async def get_calibration_by_id(self, calibration_id: UUID | None) -> Any | None: ...
```

Factory protocol passed to your `robot_builder` callable. Use `find_port_by_serial(serial_number)` to resolve a USB serial number to a port path, and `get_calibration_by_id(calibration_id)` to load robot-specific calibration data.
