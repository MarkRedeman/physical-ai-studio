...
| `_spawn_task()` | Background task set | Same | Identical implementation |
| Position streaming for 3D preview | `stream_positions` / `stop_stream` → `state_was_updated` | Same | Same event format |
| Streaming guard | `if self._streaming: return` | Same | Identical |
| `ping` / `pong` | Yes | Yes | Identical |
| `re_probe` command | Re-runs voltage + probe | Re-runs diagnostics | Same pattern |
...
| Stream on mount / stop on unmount | `useEffect` + `mountRef` | Same | 100% identical |
...
# _spawn_task — both workers, identical implementation
...
# Streaming guard pattern — identical in both
async def _handle_stream_positions(self) -> None:
    if self._streaming:
        return
    self._streaming = True
    while self._streaming and not self._stop_requested:
        # ... read positions, send state_was_updated ...
        await asyncio.sleep(0.05)
```

**Estimation:** ~120 lines of duplicated code across the two workers (~16% of
SO101 + ~32% of Trossen). A `BaseSetupWorker` class could absorb this.
...
- `reProbe`, `streamPositions`, `stopStream`, `ping` — same pattern in both
...
- Mount effect: `if (isConnected) streamPositions(); return () => stopStream()`
  using `mountRef` pattern — 100% identical.
...
**Estimation:** ~80 lines of duplicated code. The `useSyncJointState` hook
(~20 lines) should be extracted into `shared/setup-wizard/`. The mount/unmount
streaming pattern (~15 lines) could also be extracted as
`useStreamOnMount(commands, isConnected)`.
...
| Frequency | Configurable FPS (default 30) | Hardcoded `asyncio.sleep(0.05)` (~20Hz) |
...
3. Both paths share the same WebSocket lifecycle, command loop, and verification
   streaming.

A `BaseSetupWorker` that provides the lifecycle, command loop, and streaming
infrastructure — with subclasses providing the robot-specific diagnostics and
calibration steps — is the right level of abstraction.
...
- Eliminates ~120 lines of backend duplication, ~100 lines of frontend WS hook
  duplication, ~80 lines of provider duplication, ~80 lines of verification step
  duplication
...
- Reduces the surface area for bugs (one implementation of the command loop,
  one implementation of streaming, etc.)
...
class BaseSetupWorker(TransportWorker):
    ...
    Provides:
    - run() lifecycle: connect → diagnostics → command_loop → cleanup
...
### Step 3: Extract `useStreamOnMount` (frontend)

Move to `shared/setup-wizard/use-stream-on-mount.ts`:

```ts
// Accepts { streamPositions, stopStream } commands + isConnected boolean.
// Calls streamPositions on mount if connected, stopStream on unmount.
// Uses mountRef pattern to avoid re-triggering.
```

**Lines saved:** ~15 per verification step (30 total).
**Risk:** Very low.
...
- **A unified websocket hook:** The SO101 hook has 11 state fields and 13
  commands; the Trossen hook has 6 and 4. A generic hook would need extensive
...
| `useStreamOnMount` | ~30 lines | Extract to shared | Very low |
...
Total recoverable duplication: **~470 lines** across backend and frontend, of
which **~310 lines** (Steps 1–4) can be extracted with low risk. The remaining
**~160 lines** (Steps 5–6) require more careful design and can wait until
LeKiwi integration provides a third data point.
