...
const useModelUpload = (projectId: string): UseModelUploadResult => {
    ...
    const mutation = useMutation({
        mutationFn: async ({ file, modelName, datasetId, policy }: UploadVariables) => {
            ...
            await new Promise<void>((resolve, reject) => {
                ...
                xhr.onload = () => {
                    if (xhr.status >= 200 && xhr.status < 300) {
                        ...
                        reject(new Error(`Failed to import model archive: ${xhr.status}`));
                    }
                };
...
export const ImportModelDialog = ({ onClose, onImportCompleted }: ImportModelDialogProps) => {
    ...
    const onImport = async () => {
        if (!canImport || archive === null || selectedDataset === null || selectedPolicy === null) {
            return;
        }
        ...
        try {
            ...
            if (!isAbortError(error)) {
                notify('error', 'Failed to import model archive');
            }
        }
    };
                ...
                <Flex direction='column' gap='size-200'>
                    ...
                    >
                        {MODEL_POLICIES.map((policy) => (
                            <Item key={policy.id}>{policy.label}</Item>
                        ))}
                    </Picker>
                    ...
                    {mutation.isError && !isAbortError(mutation.error) ? (
                        <InlineAlert variant='negative'>
                            ...
                            <Content>Failed to upload model archive. Please try again.</Content>
                        </InlineAlert>
                    ) : null}
                </Flex>
            </Content>
