...
const commonProps = ({ name, schema, isRequired, info }: Pick<FieldProps, 'name' | 'schema' | 'isRequired' | 'info'>) => ({
    label: fieldLabel(name, schema),
    description: schema.description,
    contextualHelp: <FieldContextualHelp info={info ?? (!Array.isArray(schema['x-physicalai-ui']) ? schema['x-physicalai-ui']?.info : undefined)} />,
    isRequired,
    width: '100%' as const,
});
