...
"""Bimanual SO101 AI robot driver.

Composes two :class:`SO101` arms (left + right) behind the
:class:`~physicalai.robot.Robot` protocol, prefixing all joint names with
``left_`` / ``right_`` respectively.

Usage::

    from physicalai.robot.trossen import SO101, BimanualSO101

    left = SO101(ip="192.168.1.10", role="follower")
    right = SO101(ip="192.168.1.11", role="follower")
...
class BimanualSO101Observation:
    """Merged observation from a bimanual SO101 robot.
...
class BimanualSO101(Robot):
    """Two-arm SO101 driver composing a left and right :class:`SO101`.
    ...
    def __init__(self, left: SO101, right: SO101) -> None:
        """Initialize BimanualSO101 with two single-arm drivers.
    ...
    def get_observation(self) -> BimanualSO101Observation:
